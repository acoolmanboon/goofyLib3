...
// MAIN MODULE
GLFWwindow* goofy_initWindow(const char* windowName, int width, int height) {
    ...
    // 4.6 OPENGL and use CORE PROFILE alr nvm i know what profiles are now
    ...
    printf("[GOOFYLIB3] Sizeof vertice %d \n",sizeof(GOOFY_VERTICE));
...
GOOFY_BUFFER goofy_initBuffer(GLuint maxVertices, GLuint maxIndices) {
    GOOFY_BUFFER buffer;
...
void goofy_renderMesh(GOOFY_BUFFER* buffer, GOOFY_MESH* mesh, size_t vertexOffset, size_t indexOffset, GLuint shaderProgram) {
    // most painful 20 lines of code in my whole life
    glBindVertexArray(buffer->VAO);

    glBindBuffer(GL_ARRAY_BUFFER, buffer->VBO);
    glBufferSubData(GL_ARRAY_BUFFER, vertexOffset * sizeof(GOOFY_VERTICE), mesh->vertexCount * sizeof(GOOFY_VERTICE), mesh->vertices);

    glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, buffer->EBO);
    glBufferSubData(GL_ELEMENT_ARRAY_BUFFER, indexOffset * sizeof(unsigned int), mesh->indexCount * sizeof(unsigned int), mesh->indices);

    glActiveTexture(GL_TEXTURE0);
    glBindTexture(GL_TEXTURE_2D_ARRAY, textureArray);
    glUniform1i(glGetUniformLocation(shaderProgram, "textureArray"), 0); // Send the texture unit index to the shader

    unsigned int* adjustedIndices = malloc(sizeof(unsigned int) * mesh->indexCount);
    for (size_t i = 0; i < mesh->indexCount; ++i) {
        adjustedIndices[i] = mesh->indices[i] + vertexOffset;
    }
    glBufferSubData(GL_ELEMENT_ARRAY_BUFFER, indexOffset * sizeof(unsigned int), mesh->indexCount * sizeof(unsigned int), adjustedIndices);
    free(adjustedIndices);
    
    glDrawElementsBaseVertex(GL_TRIANGLES, mesh->indexCount, GL_UNSIGNED_INT, (void*)(indexOffset * sizeof(unsigned int)), 0);
...
void goofy_deleteBuffer(GOOFY_BUFFER* buffer) { // simply delete your buffers
    if (!buffer) return;
